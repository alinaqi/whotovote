...
const ComparisonInfographic = ({ selectedParties }) => {
...
const WhoToVote = () => {
  ...
  const [selectedParties, setSelectedParties] = useState(['CDU']);
  ...
  const togglePartySelection = (partyKey) => {
  ...
  const removeParty = (partyKey) => {
    setSelectedParties(selectedParties.filter(key => key !== partyKey));
  };

  const filteredParties = Object.entries(PARTY_DATA).filter(([key, party]) =>
